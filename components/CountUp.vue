<template>
  <span>
    <template v-if="format">{{ format(showNumber) }}</template>
    <template v-else>{{ showNumber }}</template>
  </span>
</template>
...
export default {
  ...
  props: ["speed", "times", "startNum", "endNum", "format"],
  data() {
    return {
      showNumber: 0
    };
  ...
  methods: {
    JNumberScroll(params) {
      ...
      for (var i = 0; i < defaultParams.times; i++) {
        var tempNum = add
          ? defaultParams.startNum + splitNum * i
          : defaultParams.startNum - splitNum * i;
        ...
        splitArr.push(
          isFloat
            ? parseInt(tempNum * 10 ** decimalNum) / 10 ** decimalNum
            : parseInt(tempNum)
        );
      }
      // 如果增加，并且数组最后一位比end小，则push最后一位；如果减少，并且数组最后一位比end大，则push最后一位
      ((add && splitArr[splitArr.length - 1] < defaultParams.endNum) ||
        (!add && splitArr[splitArr.length - 1] > defaultParams.endNum)) &&
        splitArr.push(defaultParams.endNum);
      ...
      var intervalId = setInterval(function() {
        if (_index < splitArr.length) _this.showNumber = splitArr[_index++];
        else {
